...
class SwipeMenuLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null,
    ...
    private var mScaleTouchSlop: Int = 0//为了处理单击事件的冲突
    ...
    private var iosInterceptFlag: Boolean = false//IOS类型下，是否拦截事件的flag
    ...
    private fun init(context: Context, attrs: AttributeSet?, defStyleAttr: Int) {
        ...
        (0 until count)
                ...
                .forEach {
                    when (it) {
                        R.styleable.SwipeMenuLayout_swipeEnable -> isSwipeEnable = ta.getBoolean(it, true)
                        R.styleable.SwipeMenuLayout_ios -> isIos = ta.getBoolean(it, true)
                        R.styleable.SwipeMenuLayout_leftSwipe -> isLeftSwipe = ta.getBoolean(it, true)
                    }
                }
        ta.recycle()
    }
    ...
    private fun forceUniformHeight(count: Int, widthMeasureSpec: Int) {
        ...
        val uniformMeasureSpec = View.MeasureSpec.makeMeasureSpec(measuredHeight,
                View.MeasureSpec.EXACTLY)//以父布局高度构建一个Exactly的测量参数
